...
bool Zombie::getAttacked(int dir)  // return true if dies
{
    ...
    if(m_injured) 
        return true;
     switch(dir){
        case UP: if(m_row > 1){m_row--; break;} return true;
        case DOWN: if(m_row < m_arena->rows()){m_row++; break;} return true;
        case LEFT: if(m_col > 1){m_col--; break;} return true;
        case RIGHT: if(m_col < m_arena->cols()){m_col++; break;} return true;
    }
    m_injured = true;
    return false;
}
...
void Player::moveOrAttack(int dir)
{
    ...
    int r = m_row, c = m_col;
    switch(dir){
        case UP: if(r > 1) r--; break;
        case DOWN: if(r < m_arena->rows()) r++; break;
        case LEFT: if(c > 1) c--; break;
        case RIGHT: if(c < m_arena->cols()) c++; break;
    }
...
int Arena::numZombiesAt(int r, int c) const
{
    ...
    return l;  // This implementation compiles, but is incorrect.
}
...
bool Arena::determineNewPosition(int& r, int& c, int dir) const
{
      ...
      // return true.
      switch(dir){
        case UP: if(r > 1){r--; break;} else{return false;} 
        case DOWN: if(r < m_rows){r++; break;} else{return false;}
        case LEFT: if(c > 1){c--; break;} else{return false;}
        case RIGHT: if(c < m_cols){c++; break;} else{return false;}
    ...
    for(r = 1; r <= rows(); r++)
        for(c = 1; c <= cols(); c++){
            if(numZombiesAt(r,c) > 0){
                ...
                grid[r-1][c-1] = numZombiesAt(r,c) + k;
